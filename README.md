...
- 「持ち物」タブにチェックリスト。自由入力で追加、チェック、削除・直前の削除取り消しに対応。免許証など12項目から開始。
- 持ち物はGitHub Pagesの端末別リストとしてブラウザに保存。再読込後も復元し、共有相手とは同期しない。保存失敗時は画面に表示。
...
Main files: app.js (UI and choices), itinerary.js (itinerary and booking notes), movement.js (maps), catalog.js and extra-catalog.js (recommendations), movement.css (transport and discovery styles), packing.js / packing.css (personal packing checklist).
